...
Fixpoint snoc_list_of_list {A} (l : list A) :=
  match l with
  | [] => ⟨⟩
  | h :: t => snoc_list_of_list t ,, h
  end.

Fixpoint SnocIn {A} (x : A) (l : snoc_list A) : Prop :=
  match l with
  | ⟨⟩ => False
  | l ,, y => SnocIn x l ∨ x = y
  end.
...
Section RWCAS.
  ...
  Proof.
    ...
    Variant tracker_inv (c : Implementation.configuration Π ReadCAS.t ReadWrite.Cell) (π : Π) : status Π ReadWrite.Cell → Type :=
      ...
      Fixpoint linearize_many (πs : snoc_list Π) (f : Π → status Π ReadWrite.Cell) :=
        match πs with
        | ⟨⟩ => f
