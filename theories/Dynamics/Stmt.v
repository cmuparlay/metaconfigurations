...
Inductive eval {Π Ω} `{Object Π Ω} (π : Π) (ψ : stringmap Value.t) (ϵ : states Π Ω) : Stmt.t Π Ω → stringmap Value.t → states Π Ω → signal → Prop :=
  ...
  | eval_goto n :
    ⟨ π , ψ , ϵ , Syntax.Stmt.Goto n ⟩ ⇓ₛ ⟨ ψ , ϵ , Goto n ⟩
  | eval_invoke ω op arg ϵ' v :
    ⟨ π , ψ , ϵ , Term.Invoke ω op arg ⟩ ⇓ₑ ⟨ ϵ' , v ⟩ →
    ⟨ π , ψ , ϵ , Invoke (Invocation ω op arg) ⟩ ⇓ₛ ⟨ ψ , ϵ' , Continue ⟩
  | eval_return e ϵ' v :
    ⟨ π , ψ , ϵ , e ⟩ ⇓ₑ ⟨ ϵ' , v ⟩ →
    ⟨ π , ψ , ϵ , Syntax.Stmt.Return e ⟩ ⇓ₛ ⟨ ψ , ϵ' , Return v ⟩
