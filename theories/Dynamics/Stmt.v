...
Section Eval.
  ...
  Inductive eval (ψ : RegisterFile.t π) (ϵ : states) : Stmt.t Ω π → RegisterFile.t π → states → signal → Prop :=
    ...
    | eval_invoke ω op arg ϵ' v :
      Term.eval ψ ϵ (Term.Invoke ω op arg) ϵ' v →
      ⧼ ψ , ϵ , Invoke ω op arg ⧽ ⇓ₛ ⧼ ψ , ϵ' , Continue ⧽
    | eval_return e ϵ' v :
      Term.eval ψ ϵ e ϵ' v →
      ⧼ ψ , ϵ , Stmt.Return e ⧽ ⇓ₛ ⧼ ψ , ϵ' , Return v ⧽
  where "⧼ ψ , ϵ , s ⧽ ⇓ₛ ⧼ ψ' , ϵ' , sig ⧽" := (eval ψ ϵ s ψ' ϵ' sig) : dynamics_scope.
End Eval.
